...
export default function TafPage() {
  ...
  const tafData = [
    ...
    {
      title: "2. IMPULSÃO HORIZONTAL",
      subtitle: "SALTO EM DISTÂNCIA",
      goal: "2,14 metros",
      metric: "Força explosiva",
      current: 1.8,
      target: 2.14,
      unit: "m",
      color: "bg-[#4A85D4]",
      borderColor: "border-[#4A85D4]"
    },
    {
      title: "3. FLEXÕES ABDOMINAIS",
      ...
      <div className="p-4 md:p-8 max-w-[1400px] mx-auto space-y-8">
        <div className="flex items-center gap-4 border-b border-[#2A2A2A] pb-5">
          ...
          <div>
            ...
            <p className="text-system-muted font-body font-semibold text-[14px] tracking-[1px] mt-2">TESTE DE APTIDÃO FÍSICA - PPRN</p>
          </div>
        </div>
        ...
        <div className="bg-[#111] border border-[#2A2A2A] rounded-lg overflow-hidden">
          <div className="p-6 border-b border-[#2A2A2A] bg-[#181818] flex items-center justify-between">
            <div>
              ...
              <p className="text-system-muted font-body font-semibold text-[14px] tracking-[1px] mt-2">4 PROVAS • PROGRESSÃO SEMANAL BASEADA EM IA</p>
            </div>
          ...
          <div className="p-6 md:p-8 bg-[#0a0a0a] min-h-[450px]">
            ...
            {activeTab === 'METAS' && (
              <div className="space-y-8">
                <div>
                  <h3 className="font-title font-bold text-[18px] text-system-muted tracking-[2px] mb-6 border-l-4 border-[#4A85D4] pl-3">PROGRESSÃO ESPERADA (90 DIAS)</h3>
                  <div className="space-y-5 max-w-3xl">
                    <div className="bg-[#111] p-5 rounded-lg border border-[#2A2A2A]">
                      <div className="flex justify-between font-body font-semibold text-[16px] text-white mb-3"><span>Barra (reps)</span> <span className="text-[#4CAF4C] font-black">3 ➔ 10</span></div>
                    ...
                    <div className="bg-[#111] p-5 rounded-lg border border-[#2A2A2A]">
                      <div className="flex justify-between font-body font-semibold text-[16px] text-white mb-3"><span>Impulsão (m)</span> <span className="text-[#4CAF4C] font-black">2,14 ➔ 2,35</span></div>
                      <div className="w-full h-3 bg-[#181818] rounded-full"><div className="h-full bg-[#00E5FF] w-4/5 rounded-full shadow-[0_0_10px_rgba(0,229,255,0.4)]"></div></div>
                    </div>
                    <div className="bg-[#111] p-5 rounded-lg border border-[#2A2A2A]">
                      <div className="flex justify-between font-body font-semibold text-[16px] text-white mb-3"><span>Abdominais (reps/min)</span> <span className="text-[#4CAF4C] font-black">31 ➔ 45</span></div>
                    ...
                    <div className="bg-[#111] p-5 rounded-lg border border-[#2A2A2A]">
                      <div className="flex justify-between font-body font-semibold text-[16px] text-white mb-3"><span>Corrida 2km (min)</span> <span className="text-[#4CAF4C] font-black">12 ➔ 10 min</span></div>
            ...
            {activeTab === 'FASE1' && (
              <div className="animate-in fade-in duration-300">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                  ...
                  <DayCard day="Terça" items={["Barra: 5 séries até a falha", "Flexão de braço: 3x10", "Remada com elástico: 3x12"]} color="border-t-[#00E5FF]" />
                  ...
                  <DayCard day="Sexta" items={["Agachamento: 3x15", "Salto horizontal: 5 tentativas", "Afundo: 3x12 cada perna"]} color="border-t-[#B026FF]" />
            ...
            {activeTab === 'FASE2' && (
              <div className="animate-in fade-in duration-300">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                  ...
                  <DayCard day="Terça" items={["Barra: 6 séries (pausa 2 min)", "Flexão diamante: 3x8", "Bíceps c/ halteres: 3x12"]} color="border-t-[#00E5FF]" />
                  <DayCard day="Quarta" items={["Corrida 25 min ritmo médio", "Salto no banco: 4x8", "Agachamento explosivo: 3x10"]} color="border-t-[#FFB800]" />
                  <DayCard day="Quinta" items={["Abdominal: 5x20 (1 min intervalo)", "Prancha lateral: 3x25 seg", "Superman: 3x15"]} color="border-t-[#B026FF]" />
                  <DayCard day="Sexta" items={["Tiro: 8x200m (pace forte)", "Salto horizontal: 8 tentativas", "Afundo com salto: 3x10"]} color="border-t-[#4CAF4C]" />
                  <DayCard day="Sábado" items={["Barra: 4 séries", "Remada invertida: 3x10", "Flexão inclinada: 3x12"]} color="border-t-[#4A85D4]" />
            ...
            {activeTab === 'FASE3' && (
              <div className="animate-in fade-in duration-300">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                  ...
                  <DayCard day="Terça" items={["Barra: 8 séries máximas", "Flexão: 4x15", "Core: 3 exercícios 3x20"]} color="border-t-[#00E5FF]" />
                  <DayCard day="Quarta" items={["Simulado de corrida 2.000m", "Salto horizontal: 10 tentativas", "Análise do tempo"]} color="border-t-[#FFB800]" />
                  <DayCard day="Quinta" items={["Abdominal: 3x40 c/ 1 min intervalo", "Agachamento explosivo: 4x8", "Alongamento"]} color="border-t-[#B026FF]" />
                  <DayCard day="Sexta" items={["TAF COMPLETO SIMULADO", "Todas as 4 provas em sequência", "Registre os tempos"]} color="border-t-[#FF0000] shadow-[0_0_15px_rgba(255,0,0,0.2)]" />
                ...
                <div className="mt-8 bg-[#3D0000] border border-[#FF0000] p-5 rounded-lg flex gap-4 items-start shadow-lg">
                  ...
                  <p className="font-body font-semibold text-[15px] leading-relaxed text-[#FFBABA]">Última semana (Sem 13): reduza o volume em 50%. Durma bem, mantenha hidratação e chegue descansado no dia da prova. A sequência das provas no dia é: Barra → Impulsão → Abdominal → Corrida.</p>
                </div>
              </div>
            )}
            ...
            {activeTab === 'DICAS' && (
              <div className="space-y-6 animate-in fade-in duration-300 max-w-5xl">
                <TipCard 
                  title="BARRA — TÉCNICA PARA GANHAR REPETIÇÕES" 
                  desc="Use pegada pronada (palmas para frente), cotovelos em direção ao chão na subida. Treine negativa (desça devagar em 4 seg) para construir força rápido. Se não consegue nenhuma repetição, comece com flexão inclinada e remada invertida."
                />
                <TipCard 
                  title="IMPULSÃO — COMO AUMENTAR O SALTO" 
                  desc="Trabalhe agachamento, afundo com salto e caixa de pliometria. Na prova, faça balanço de braços e agachamento rápido antes de saltar. Pratique o salto 3-4 vezes por semana com 8-10 tentativas."
                />
                <TipCard 
                ...
                />
                <TipCard 
                  title="CORRIDA — ESTRATÉGIA DE PROVA" 
                  desc="Saia em ritmo controlado (não estoure no início). Para 2 km em 12 min, você precisa de um pace de 6 min/km. Treine pace de 5-5,5 min/km no treino para ter folga. Use os últimos 300 m para dar tudo."
                />
                <TipCard 
